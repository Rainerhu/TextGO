...
本仓库 fork 自 [C5H12O5/TextGO](https://github.com/C5H12O5/TextGO)，仅供个人使用，基于原项目新增了以下功能：

### 文本选择增强
- **延迟读取选中文本**：鼠标选中文本时不立即读取，等点击工具栏操作后才获取，避免在禁止复制的网站上触发检测
- **仅原生 API 模式**：跳过剪贴板回退（模拟 Ctrl+C），完全通过系统无障碍 API 读取选中文本，彻底避免触发浏览器 copy 事件
- **Windows UIA 增强**：当焦点元素无 TextPattern 时，自动搜索窗口内 Document 控件获取选中文本，提高原生 API 成功率

### 规则管理
- **单条规则启禁用**：每条规则可独立开关，禁用后不参与匹配，UI 上半透明显示
- **按应用过滤规则**：每条规则可设置 `仅在应用` 和 `排除应用`，支持通配符匹配，实现规则级别的应用过滤

### 工具栏
- **文件夹分组**：规则可归入文件夹，工具栏中显示为文件夹按钮，hover 展开子规则；文件夹支持自定义图标（Phosphor 图标库 + SVG 上传）和显示模式（图标/文字/图标+文字）
- **垂直布局**：工具栏支持水平/垂直两种布局方向，每个快捷键独立配置

### AI 对话
- **可调尺寸弹窗**：AI 对话弹窗支持鼠标拖拽调整大小，可选记忆尺寸，可配置默认宽高
- **一键复制输出**：AI 对话模式标题栏新增复制按钮，一键复制 AI 响应的 Markdown 原文

### 数据管理
- **完整导入导出**：在通用设置中一键导出所有配置（快捷键、规则、脚本、提示词、搜索动作、设置等）为 JSON 文件，支持从 JSON 文件导入恢复（不含 API 密钥）
